 ...
 */
public final class SoftCommitCommand extends AbstractCommand {
    /** logger. */
    private static final Log LOG = LogFactory.getLog(SoftCommitCommand.class);

    /** property: anuenue.http.connect.timeout.optimize. */
    private static final String PROPERTY_HTTP_READ_TIMEOUT_COMMIT = "anuenue.http.read.timeout.softcommit";
     ...
     * @param arg argument
     */
    public SoftCommitCommand(final SolrClient client, final String arg) {
        super(client, arg);
    ...
    public String runCommand() throws IOException {
        LOG.info("soft commiting the changes");
        return getSolrClient().updateXML(AnuenueCommandConstants.SOFT_COMMIT_COMMAND);
    }
